...
! ------------------------------------------------------------------------------
    module subroutine newt1var_solve(this, fcn, x, lim, f, ib, err)
        ...
        real(real64) :: ftol, xtol, xh, xl, fh, fl, x1, x2, eps, rts, dxold, &
            dx, df, temp, ff
        ...
        do
            ...
            if ((((rst - hs) * df - ff) * ((rts - xl) * df - ff) > zero) .or. &
                (abs(two * ff) > abs(dxold * df))) &
            then
                dxold = dx
                dx = p5 * (xh - xl)
                rts = xl + dx
                if (abs(xl - rts) < xtol) then
            ...
            ! Compute the next solution via Newton's method
